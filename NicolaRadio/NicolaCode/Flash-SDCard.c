...
// temp
//s32 XSdPs_CfgInitializeC(XSdPs *InstancePtr, XSdPs_Config *ConfigPtr,
//				u32 EffectiveAddr);
...
s32 XSdPs_SdCardInitializeX(XSdPs *InstancePtr)
{
	u32 PresentStateReg;
	s32 Status;
	u32 RespOCR;
...
s32 XSdPs_CardInitializeX(XSdPs *InstancePtr) {
	u8 Tmp;
	u32 Cnt;
	u32 PresentStateReg;
	u32 CtrlReg;
...
static u8 ExtCsd[512] __attribute__ ((aligned(32)));
	...
	if (InstancePtr->CardType == XSDPS_CARD_SD) {
		Status = XSdPs_SdCardInitializeX(InstancePtr);
		if (Status != XST_SUCCESS) {
			Status = XST_FAILURE;
			goto RETURN_PATH;
		}
...
void CDFlashRead( u8 *writeToAddress, u32 flashAddress, u32 numberOfBytes)
{
	...
	u32 blockCount = numberOfBytes/512;
...
s32 XSdPs_Change_ClkFreqX(XSdPs *InstancePtr, u32 SelFreq)
{
	u16 ClockReg;
	u16 DivCnt;
	u16 Divisor = 0U;
...
s32 XSdPs_CfgInitializeX(XSdPs *InstancePtr, XSdPs_Config *ConfigPtr,
				u32 EffectiveAddr)
{
	s32 Status;
	u8 PowerLevel;
	...
	Status = XSdPs_Change_ClkFreqX(InstancePtr, XSDPS_CLK_400_KHZ);
	if (Status != XST_SUCCESS) {
		Status = XST_FAILURE;
		goto RETURN_PATH ;
	}
