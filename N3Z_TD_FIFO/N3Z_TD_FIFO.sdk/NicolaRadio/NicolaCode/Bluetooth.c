...
u8 		BluetoothCommandEcho = 0;		/* echo commands send to Bluegiga to console */
...
static void BluetoothTimer2Callback( TimerHandle_t theHandle)
{
	...
	/* pass the Bluegiga message to the main task to be processed  Graham */
	while ( xQueueSendToBack( BluegigaReceiveQueue, &theMessage, 0 ) != pdPASS )
	{
		vTaskDelay( pdMS_TO_TICKS( 3 ));
	}
